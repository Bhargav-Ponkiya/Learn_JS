...
form.addEventListener("submit", function (e) {
  // prevent default behaviour when we submit form - it automatically get refreshed and so we lost our data
  e.preventDefault();

  const numOfParagraphs = parseInt(amount.value);
  ...
  if (isNaN(numOfParagraphs) || numOfParagraphs < 0 || numOfParagraphs > 9) {
    resultText.innerHTML = `<p class="result">${text[randomNumber]}</p>`;
  } else {
    let sliceArray = text.slice(0, numOfParagraphs);
    sliceArray = sliceArray
      .map(function (item) {
        return `<p class="result">${item}</p>`;
      })
      .join("");
    resultText.innerHTML = sliceArray;
  }
});
